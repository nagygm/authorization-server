        ...
        http
            .csrf().requireCsrfProtectionMatcher(NegatedServerWebExchangeMatcher { exchange ->
                ServerWebExchangeMatchers.pathMatchers(
                    "${OAuth2Api.oauth2}/**", "/login**", "/consent**", "/swagger-ui/**", "/v3/api-docs/**", "/webjars/swagger-ui/**", "/swagger-ui.html", *tempAuthOverride()
                ).matches(exchange)
            }).and()
            .authorizeExchange()
            .pathMatchers("${OAuth2Api.oauth2}/**", "/favicon.ico", "/login", "/swagger-ui/**", "/v3/api-docs/**", "/webjars/swagger-ui/**", "/swagger-ui.html", *tempAuthOverride())
            .permitAll().and()
            .authorizeExchange()
